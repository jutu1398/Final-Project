...
import torch
from .nonlocal_block import gaussian_nonlocal
import torchvision
...
#         self.model_subset = nn.Sequential(model.conv1,
#             model.bn1,
#             model.relu,
#             model.maxpool,
#             model.layer1,
...
#             model.layer3,
#     )
#         for param in self.model_subset.parameters():
#             param.requires_grad = False
        ...
        self.stage3_conv1 = nn.Sequential(nn.Conv2d(1024 + 256, 1024, 3, 1, 1, bias=False), nn.BatchNorm2d(1024),
                                          nn.LeakyReLU(0.1, inplace=True))
        self.stage3_conv2 = nn.Conv2d(1024, len(self.anchors) * (5 + num_classes), 1, 1, 0, bias=False)

    def forward(self, input):
       # output = self.model_subset(input) 
        ...
        output_2 = output_2.reshape(batch_size, -1, int(height // 2), int(width // 2))
